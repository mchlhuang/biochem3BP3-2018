...
* *Salmonella* infections are often treated without antibiotics, except for severe cases in which trimethoprim (pediatric) or flouroquinolones (adult) are used. If you find evidence of clonal outbreaks in your data, sample a genome from each subset for Resistance Gene Identifier analysis (part of http://card.mcmaster.ca). 
...
**Question #9 (3 points). *Salmonella* infections are often treated without antibiotics, except for severe cases in which trimethoprim (pediatric) or flouroquinolones (adult) are used. If you find evidence of independent sub-clades in your RAxML tree (i.e. separated by strong bootstrap support and possibly reflecting strain lables), sample a genome from each subset for Resistance Gene Identifier analysis. What is the predicted susceptibility to trimethoprim or flouroquinolones? Could you use either drug for treatment of these sub-clades?**

**Question #10 (2 points). For the single patient that is abnormally resistant to antibiotics, does the AMR++ analysis concur that resistance to drugs beyond trimethoprim or flouroquinolones exists in this microbiome? Which additional drug classes may not work against this microbiome?**
